          ...
          {reviews.map((review) => {
            ...
            return (
              <motion.button
                ...
                className="text-left w-full flex flex-col bg-slate-50 dark:bg-slate-900/50 p-8 rounded-[32px] border border-slate-100 dark:border-slate-800 relative hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 active:scale-[0.98] group cursor-pointer h-full"
              >
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex text-primary">
                      {[...Array(review.rating)].map((_, i) => (
                        <span key={i} className="material-symbols-outlined text-[20px] fill-current" aria-hidden="true">
                          star
                        </span>
                      ))}
                    </div>
                ...
                <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800/50">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-widest group-hover:text-slate-900 dark:group-hover:text-primary transition-colors leading-tight">
                          {review.name}
                        </h4>
                    ...
                    {isTruncated && (
                      <div className="shrink-0">
                        <span className="inline-flex items-center px-4 py-2 rounded-xl bg-white dark:bg-slate-800 text-slate-900 dark:text-primary font-black text-[10px] uppercase tracking-[0.15em] shadow-sm border border-slate-100 dark:border-slate-700 group-hover:bg-primary group-hover:text-white group-hover:border-primary transition-all duration-300">
      ...
      <section className="w-full px-8 py-8 mb-8">
        ...
        >
          ...
          <div className="relative z-10 flex flex-col gap-6 items-center">
            <h2 className="text-white text-3xl font-black tracking-tight">{t('cta.title', { ns: 'reviews' })}</h2>
            <p className="text-slate-300 text-lg max-w-[600px]">{t('cta.description', { ns: 'reviews' })}</p>
            <div className="flex flex-row gap-4 mt-2 w-full max-w-md justify-center">
              <Link to="/contact" className="flex items-center justify-center rounded-lg bg-primary hover:bg-primary/80 transition-all h-12 px-8 text-white text-base font-bold shadow-lg">
                {t('cta.book', { ns: 'reviews' })}
              </Link>
              <Link to="/services" className="flex items-center justify-center rounded-lg bg-white/10 border border-white/20 hover:bg-white/20 transition-colors h-12 px-8 text-white text-base font-bold backdrop-blur-sm">
