...
const ReviewsMobile: React.FC = () => {
  ...
  return (
    <>
      <section className="w-full bg-white dark:bg-slate-900 py-10 px-4 border-b border-slate-100 dark:border-slate-800 overflow-hidden">
        <motion.div
          initial={{ opacity: 0, y: 18 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          className="max-w-[960px] mx-auto text-center flex flex-col gap-3"
        >
          <div className="inline-flex items-center justify-center gap-2 px-3 py-1 rounded-full bg-yellow-50 dark:bg-yellow-900/20 w-fit mx-auto">
            <span className="material-symbols-outlined text-yellow-600 dark:text-yellow-500 text-sm" aria-hidden="true">
              favorite
            </span>
          ...
          <p className="text-slate-600 dark:text-slate-400 text-base font-light leading-relaxed max-w-xl mx-auto">
            {t('hero.description', { ns: 'reviews' })}
          </p>
        </motion.div>
      </section>

      <section className="w-full max-w-[960px] mx-auto px-4 py-10">
        <motion.div
        ...
        >
          {reviews.map((review) => (
            <motion.button
              ...
              className="text-left w-full flex flex-col bg-slate-50 dark:bg-slate-900/50 p-6 rounded-[24px] border border-slate-100 dark:border-slate-800 active:scale-[0.99] transition-transform"
            >
              <div className="flex items-center justify-between">
                <div className="flex text-primary">
                  {[...Array(review.rating)].map((_, i) => (
                    <span key={i} className="material-symbols-outlined text-[18px] fill-current" aria-hidden="true">
                      star
                    </span>
                  ))}
                </div>
              ...
              <p className="mt-4 text-slate-700 dark:text-slate-300 text-sm leading-relaxed italic line-clamp-4">
                "{review.text}"
              </p>
              ...
              <div className="mt-5 pt-5 border-t border-slate-100 dark:border-slate-800/50 flex items-center justify-between gap-4">
                ...
                <span className="inline-flex items-center px-4 py-2 rounded-xl bg-white dark:bg-slate-800 text-primary font-black text-[10px] uppercase tracking-[0.15em] shadow-sm border border-slate-100 dark:border-slate-700">
      ...
      <section className="w-full px-4 pb-12">
        <div className="max-w-[960px] mx-auto bg-slate-900 dark:bg-slate-800 rounded-2xl p-8 text-center shadow-xl relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-1 bg-primary" />
          <div className="relative z-10 flex flex-col gap-5 items-center">
            <h2 className="text-white text-2xl font-black tracking-tight">{t('cta.title', { ns: 'reviews' })}</h2>
            <p className="text-slate-300 text-base">{t('cta.description', { ns: 'reviews' })}</p>
            <div className="flex flex-col gap-3 w-full">
              <Link to="/contact" className="flex items-center justify-center rounded-lg bg-primary hover:bg-primary/80 transition-colors h-12 px-8 text-white text-base font-bold shadow-lg w-full">
                {t('cta.book', { ns: 'reviews' })}
              </Link>
              <Link to="/services" className="flex items-center justify-center rounded-lg bg-white/10 border border-white/20 hover:bg-white/20 transition-colors h-12 px-8 text-white text-base font-bold backdrop-blur-sm w-full">
